...
function renderDraw(data) {
  console.log(data);
  result.innerHTML = `<article class="draw">
          <header class="draw-header">
            <div>
              <h2 class="draw-title">Concurso ${data.concurso}</h2>
              <span class="draw-date">Data: ${formatDate(data.data_do_sorteio)}</span>
            </div>
          <strong>
...
async function loadConcurso(concurso = "") {
  ...
  await delay(500); // simula um pequeno atraso para melhor UX
  ...
  try {
    ...
    if (!response.ok) {
      setMessage(data.message || "Concurso não encontrado. ");
      return;
    }

    renderDraw(data);
  } catch (error) {
    setMessage("Não foi possível conectar à API. Tente novamente mais tarde.");
  } finally {
